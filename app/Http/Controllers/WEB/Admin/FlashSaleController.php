...
class FlashSaleController extends Controller
{
    ...
    public function store(Request $request){
        ...
        $fs = FlashSale::where('id', 1)->first();
        $offer_amount = $fs->offer;
        
        $product_data = Product::where('id', $request->product_id)->first();
        $product_price = $product_data->price;
        $flash_sell_amount = ($offer_amount*$product_price)/100;
        $flash_dis_amount = $product_price - $flash_sell_amount;
        ...
        $product_data->offer_price = $flash_dis_amount;
        $product_data->save();
